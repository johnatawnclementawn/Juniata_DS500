...
```{r}
ggplot(data = profiles) +geom_point(mapping=aes(x = age, y = height))
...
```{r}
ggplot(data = profiles) +geom_point(mapping=aes(x = age, y = height, color = sex))
```


3. Make a scatterplot of age vs. height. Size by income.
```{r}
ggplot(data = profiles) +geom_point(mapping=aes(x = age, y = height, size = income))
```
...
geom_smooth makes a best fit curve
...
```{r}
ggplot(data = profiles) +geom_point(mapping=aes(x = age, y = height)) +geom_smooth(mapping=aes(x =height, y=income ), method="lm", se=0)
```
...
```{r}
ggplot(data = profiles) +geom_bar(mapping = aes(x=drinks))
```


6. Make a bar chart of drinks and color the bars by sex using fill.
```{r}
ggplot(data = profiles) +geom_bar(mapping = aes(x=drinks,fill=sex))
```

7. Make a histogram of income. 
```{r}
ggplot(data = profiles) +geom_histogram(mapping = aes(x=income))
```
...
```{r}
ggplot(data = profiles) +geom_histogram(mapping = aes(x=income), fill = 'orange', color = 'blue')
```

9. Make a boxplot of income by sex. For a boxplot the x aes has to be the categorical/discrete variable.
```{r}
ggplot(data=profiles) +geom_boxplot(mapping = aes(x=sex, y=income))
```
...
Answer: The facet_wrap situates the sexual orietntations in a single vertical column.
